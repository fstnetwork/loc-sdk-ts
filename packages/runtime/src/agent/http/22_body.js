// reference: copy from https://github.com/denoland/deno/blob/v1.33.1/ext/fetch/22_body.js
...
import {
  ...
  formDataToBlob,
  parseFormData,
} from 'ext:saffron_fetch/21_formdata.js';
...
class InnerBody {
  ...
  constructor(stream) {
    ...
    this.streamOrStatic = stream ?? { body: new Uint8Array(), consumed: false };
  ...
  unusable() {
    if (
      ObjectPrototypeIsPrototypeOf(ReadableStreamPrototype, this.streamOrStatic)
    ) {
      return (
        this.streamOrStatic.locked ||
...
function mixinBody(prototype, bodySymbol, mimeTypeSymbol) {
  async function consumeBody(object, type) {
    ...
    const body =
      object[bodySymbol] !== null
        ? await object[bodySymbol].consume()
        : new Uint8Array();

    const mimeType =
      type === 'Blob' || type === 'FormData' ? object[mimeTypeSymbol] : null;
      ...
      if (mimeType !== null) {
        ...
        if (essence === 'multipart/form-data') {
          ...
          return formDataFromEntries(
            ArrayPrototypeMap(entries, (x) => ({ name: x[0], value: x[1] }))
          );
        }
...
function extractBody(object) {
  ...
  } else if (ObjectPrototypeIsPrototypeOf(URLSearchParamsPrototype, object)) {
