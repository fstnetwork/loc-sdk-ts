// reference: copy from https://github.com/denoland/deno/blob/v1.33.1/ext/fetch/23_request.js and edited
...
import { extractBody, mixinBody } from 'ext:saffron_fetch/22_body.js';
...
} from 'ext:saffron_fetch/20_headers.js';
import { HttpClientPrototype } from 'ext:saffron_fetch/22_http_client.js';
 ...
 */
function newInnerRequest(method, path, headerList, body, maybeBlob) {
  // NOTE: Saffron Runtime does not support Blob URL, comment it out
  ...
  return {
    ...
    redirectMode: 'follow',
    redirectCount: 0,
    urlList: [typeof path === 'string' ? () => path : path],
    urlListProcessed: [],
    clientRid: null,
    blobUrlEntry: null,
    url() {
...
class Request {
  ...
  constructor(input, init = {}) {
    ...
    if (typeof input === 'string') {
      request = newInnerRequest('GET', input, () => [], null, true);
    } else {
      // 6.
