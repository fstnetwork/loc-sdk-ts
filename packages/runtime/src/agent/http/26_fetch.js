// reference: copy from https://github.com/denoland/deno/blob/v1.33.1/ext/fetch/26_fetch.js and edited
...
import { extractBody, InnerBody } from 'ext:saffron_fetch/22_body.js';
import {
  processUrlList,
  toInnerRequest,
} from 'ext:saffron_fetch/23_request.js';
import {
  abortedNetworkError,
  fromInnerResponse,
  networkError,
  ...
  redirectStatus,
  toInnerResponse,
} from 'ext:saffron_fetch/23_response.js';
...
async function mainFetch(configurationId, req, recursive, terminator) {
  ...
  let resp;
  try {
    resp = await opFetchSend(configurationId, requestRid);
  } catch (err) {
  ...
  // NOTE: Saffron Runtime Http Agent will implement the redirect policy
